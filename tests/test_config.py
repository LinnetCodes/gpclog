...
class TestGPCLoggerConfig:
    ...
    def test_invalid_level_raises_error(self) -> None:
        """Test that invalid level is accepted (validation done elsewhere)."""
        # Level validation is done at logger creation time, not config time
        config = GPCLoggerConfig(
            configured_class_name="GPCLogger",
            level="INVALID",
        )
